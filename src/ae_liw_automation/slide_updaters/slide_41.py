...
def slide_41_updater(meta, df, df_labeled, prs):
    slide_index = 40
    print(
    ...
    base_question = 'Q27'
    question = 'Q28_1'
    label_sub_dict = {'Inclease energy savings, lower bill' : 'Increase energy savings, lower bill',
                      }
    last_rows = ['Nothing, satisfied', 'Do not know, not familiar with program', 'All other', 'Base:']
    ...
    # question_result_df.loc['Base:'] = df['Q27'].isin([1, 2, 3, 4, 5, 6, 7]).value_counts()[True]
    question_result_df.loc['Base:'] = df['Q27'].isin([1, 2, 3, 4, 5, 6, 7, 11]).value_counts().get(True, 0)
    ...
    print(f'{current_quarter_result_df_combined = }')
